...
> AI コーディングタスクのベンダー中立な handoff — Claude、GPT、Gemini、DeepSeek、ローカル LLM の間で。
> Claude Code、Codex、Cursor、Aider のセッションを読み取り。Local-first。

![drift handoff demo](docs/demo/v040-handoff-bidirectional.gif)

---

## 🧠 AI agent を切り替えるたびに開発が中断する

Claude が止まる。Codex が拒否する。Cursor が脱線する。

すでに片付けたはずの **30 分ぶんの文脈を再説明** することになります。

- どんな判断をしたか？
- どんな方法を試して失敗したか？
- どのファイルを編集していたか？

👉 これらは session が終わると全部失われます。

---

## ❌ Git はコードを追跡するだけで、AI の推論は追跡しない
...
デフォルトでは `db_in_git = true`(チームは repo 経由で blame を共有)
です。ソロ repo や機密性の高い内容では `false` に切り替えてください — 詳しくは
[Configuration](#configuration) を参照。

将来のリリースで regex ベースの redaction pass が追加されます。今すぐ完全
にカバーしたい場合は、`drift` を
[gitleaks](https://github.com/gitleaks/gitleaks) や
[trufflehog](https://github.com/trufflesecurity/trufflehog) と組み合わせ
て pre-commit hook として使ってください。

> **AI session に日常的に secret を貼り付けているなら、redaction pass
> がリリースされるまで共有 repo で `drift` を有効にしないでください。**
...
drift handoff --branch feature/oauth --to claude   # task transfer (v0.2 から)
